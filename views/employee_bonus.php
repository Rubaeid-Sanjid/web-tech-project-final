<?php
session_start();
if (!isset($_COOKIE['username']) || $_COOKIE['role'] != 'employee') {
    header('Location: login.php');
    exit();
}
 
$taskNum = $_SESSION['taskNum'];
 
$bonus = 0;
$referenceUsername = '';
 
// Handle username validation for the reference
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['referenceUsername'])) {
    $referenceUsername = $_POST['referenceUsername'];
 
    // Connect to the database
    $conn = new mysqli('localhost', 'root', '', 'ecommerce');
 
    // Check the connection
    if ($conn->connect_error) {
        die('Connection failed: ' . $conn->connect_error);
    }
 
    // Validate the username in the database
    $stmt = $conn->prepare("SELECT username, bonus_points FROM users WHERE username = ?");
    $stmt->bind_param('s', $referenceUsername);
    $stmt->execute();
    $result = $stmt->get_result();
 
    if ($result->num_rows > 0) {
        // Username exists, fetch the current bonus points
        $user = $result->fetch_assoc();
        $currentBonus = $user['bonus_points'] ?? 0;
 
        // Award the bonus to the referred user
        $newBonus = $currentBonus + 20;
 
        // Update the user's bonus points
        $updateStmt = $conn->prepare("UPDATE users SET bonus_points = ? WHERE username = ?");
        $updateStmt->bind_param('is', $newBonus, $referenceUsername);
        if ($updateStmt->execute()) {
            $bonus = 20; // Only set the bonus for display purposes
        }
        $updateStmt->close();
    }
 
    $stmt->close();
    $conn->close();
}
?>
 
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../styles/employee_utilities.css">
    <link rel="stylesheet" href="../styles/employee_bonus.css">
    <title>Employee Dashboard</title>
</head>
<body>
    <div class="sidebar">
        <a href="../controllers/employee_dashboard_controller.php"><h1>Employee Dashboard</h1></a>
        <a href="../controllers/update_user.php"><h2>Update Profile</h2></a>
        <h2>Attendance Management</h2>
        <ul>
            <li><a href="../controllers/mark_attendance_controller.php">Mark attendance</a></li>
            <li><a href="../controllers/view_attendance_employee_controller.php">View attendance</a></li>
        </ul>
        <h2>Task Management</h2>
        <ul>
            <li><a href="../controllers/report_task_controller.php">Report task</a></li>
            <li><a href="#"></a></li>
        </ul>
        <a href="../controllers/employee_bonus_controller.php"><h2>Employee Bonus</h2></a>
        <a href="../controllers/salary_sheet_controller.php"><h2>Salary Sheet</h2></a>
        <a href="../controllers/employee_notification_controller.php"><h2>Notification</h2></a>
        <a href="../controllers/logout.php"><h2>Logout</h2></a>
    </div>
    <div class="container">
        <h1 class="profile">Employee Bonus</h1>
        <div class="info">
            <p>Number of completed tasks:</p>
            <p><?= $taskNum ?></p>
        </div>
        <div class="info">
            <p>Bonus per task:</p>
            <p>$50</p>
        </div>
        <div>
            <p>Total Bonus:</p>
            <p class="bonus" id="total-bonus"><?= '$' . ($taskNum * 50 + $bonus) ?></p>
        </div>
        <form method="POST" id="referenceForm">
            <button type="button" id="referenceButton">Reference</button>
            <input type="text" name="referenceUsername" id="referenceUsername" placeholder="Enter username" style="display: none;" required>
            <button type="submit" id="submitButton" style="display: none;">Submit</button>
        </form>
        <div id="referenceResult">
            <?= $referenceUsername ? "Reference Username: $referenceUsername (Referred user bonus awarded: $20)" : '' ?>
        </div>
    </div>
</body>
 
<script>
document.addEventListener('DOMContentLoaded', () => {
    const referenceButton = document.getElementById('referenceButton');
    const referenceInput = document.getElementById('referenceUsername');
    const submitButton = document.getElementById('submitButton');
 
    referenceButton.addEventListener('click', () => {
        referenceInput.style.display = 'inline-block';
        submitButton.style.display = 'inline-block';
    });
});
</script>
</html>